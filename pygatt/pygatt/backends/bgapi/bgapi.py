...
from .device import BGAPIBLEDevice, BGAPIBLEPeripheral
...
def response_pending(func):
    """
        Wait until a pending response is handled. 
    """
    def wrapper(self, *args, **kwargs):
        while self._resp_pending:
            #hmk TODO: time.sleep(0.01)
            pass
...
class BGAPIBackend(BLEBackend):
    ...
    def _open_serial_port(self,
        ...
        for attempt in range(max_connection_attempts):
            ...
            try:
                ...
                self._ser = None
                log.info("Opening serial port (port %s)",
                          serial_port)
        ...
        while self._running.is_set():
            ...
            if packet is not None:
                ...
                if packet_type == EventPacketType.attclient_attribute_value:
                    ...
                    if None != self._event_cb:
                        self._event_cb(event_type, args)
                    ...
                    if None != self._peripheral:
                        self._peripheral.notify_write(packet)
                elif packet_type == EventPacketType.attributes_user_read_request:
                    if None != self._peripheral:
                        self._peripheral.notify_read(packet)
    ...
    #--------------------------------------------------------------------------------
    # Additional BLE server functions
    #--------------------------------------------------------------------------------
    def attach(self, event_cb=None):
        self._event_cb = event_cb
        self._peripheral = BGAPIBLEPeripheral(self)
    ...
    @response_pending
    def set_adv_data(self, con_adv_pack=[], scan_resp_pack=[]):
        if 0 < len(con_adv_pack):
            # set connectable_advertisement_packet
            self.send_command(
                CommandBuilder.gap_set_adv_data(
                    0x00, con_adv_pack))
            self.expect(ResponsePacketType.gap_set_adv_data)
        
        if 0 < len(scan_resp_pack):
            # set scan_response_packet data
            self.send_command(
                CommandBuilder.gap_set_adv_data(
                    0x01, scan_resp_pack))
            self.expect(ResponsePacketType.gap_set_adv_data)
    ...
    @response_pending
    def set_adv_parameter(self, interval_min=1000, interval_max=1500, channels=0x07):
        self.send_command(
            CommandBuilder.gap_set_adv_parameters(
                interval_min, interval_max, channels))
        self.expect(ResponsePacketType.gap_set_adv_parameters)
        
    @response_pending
    def start_advertising(self, adv_mode=constants.gap_discoverable_mode['user_data'],
                                conn_mode=constants.gap_connectable_mode['undirected_connectable']):
        self.send_command(
            CommandBuilder.gap_set_mode(
                adv_mode, conn_mode))
        self.expect(ResponsePacketType.gap_set_mode)
