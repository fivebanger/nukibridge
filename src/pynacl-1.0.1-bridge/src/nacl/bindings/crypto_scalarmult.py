...
def crypto_scalarmult_curve25519(n, p):
    ...
    :rtype: bytes
    """
    q = ffi.new("unsigned char[]", crypto_scalarmult_curve25519_BYTES)

    rc = lib.crypto_scalarmult_curve25519(q, n, p)
    assert rc == 0

    return ffi.buffer(q, crypto_scalarmult_curve25519_BYTES)[:]